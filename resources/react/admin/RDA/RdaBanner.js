...
import { GetRdaBanners, DeleteRdaBanner, UpdateRdaBannerStatus } from '../../store/rda/api';
import RdaBannerForm from './RdaBannerForm';
...
const RdaBanner = () => {
    ...
    const getData = (state) => {
        ...
        GetRdaBanners({...state, type}).then(res => {
            setSubmitting(false);
                    ...
                    banner: banner.image ? (
                        <>
                            {fileExtension(banner.image) === 'mp4' ?
                                <video style={{height: '36px'}}>
                                    <source src={banner.image} type="video/mp4"/>
                                </video>
                                :
                                <img src={banner.image} style={{height: '36px'}}/>
                            }
                        </>
                    ) : '',
    ...
    const handleDeleteRdaBanner = () => {
        if(submitting) return;
        setSubmitting(true);

        DeleteRdaBanner(selectedId).then(res => {
            setSubmitting(false);
            setSelectedId(null);
    ...
    const handleChangeStatus = (banner) => {
        UpdateRdaBannerStatus(banner.id, {status: banner.status === 'ACTIVE' ? 'DISABLE' : 'ACTIVE'})
        .then(res => {
            getData(tableState);
        }).catch(err => {
            console.log(err);
        ...
        <div className="pb-4">
            ...
            {selectedId && 
                <SweetAlert
                    ...
                    cancelBtnText="취소"
                    title="삭제하시겠습니까?"
                    onConfirm={handleDeleteRdaBanner}
