    ...
    const handleSubmit = () => {
        ...
        PostApplicant(data.id, _data)
            .then(res => {
                ...
                history.push('/admin/wmm_camp_applicants');
            }).catch(err => {
                setSubmitting(false);
            })
    }
    ...
    return (
        <div className="pb-4">
            ...
            <div className="d-flex px-1 justify-content-center mt-5">
                ...
                <Button label={data.id ? '돌아가기' : '취소하기'} className="light px-5 mx-2" onClick={() => history.push('/admin/wmm_camp_applicants')}/>
            </div>
        </div>
    )
};

export default WMMCampApplicant;