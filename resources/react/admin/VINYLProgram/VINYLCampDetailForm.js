...
const VINYLCampDetailForm = ({campInfo, open, onClose, onSuccess}) => {
    const type = 'VINYL';
    ...
    const getEditorData = () => {
        if(editor){
            editor.getById['vinyl-camp-content'].exec("UPDATE_CONTENTS_FIELD", []);
            const content = document.getElementById('vinyl-camp-content').value;
            return content;
        }
        return '';
    }
...
export default VINYLCampDetailForm;