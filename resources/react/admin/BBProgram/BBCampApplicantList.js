...
const BBCampApplicantList = () => {
        ...
        GetApplicants({...state, type}).then(res => {
            ...
            let _applicants = applicants.map((applicant, index) => {
                return {
                    ...
                    applicant_id: <Link to={`/admin/beatbox_camp_applicant_detail/${applicant.id}`} className="text-decoration-underline cursor-pointer">
    ...
    const handleGoToDetail = (item) => {
        if(item) history.push(`/admin/beatbox_camp_applicant/${item.id}`);
        else history.push(`/admin/beatbox_camp_applicant`)
    }
    ...
    return (
        <div>
            ...
            <h4 className="mb-5">BEATBOX 신청자 목록</h4>
...
export default BBCampApplicantList;