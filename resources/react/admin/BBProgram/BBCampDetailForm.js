...
const BBCampDetailForm = ({campInfo, open, onClose, onSuccess}) => {
    const type = 'BEATBOX';
    ...
    const getEditorData = () => {
        if(editor){
            editor.getById['bb-camp-content'].exec("UPDATE_CONTENTS_FIELD", []);
            const content = document.getElementById('bb-camp-content').value;
            return content;
        }
        return '';
    }
...
export default BBCampDetailForm;