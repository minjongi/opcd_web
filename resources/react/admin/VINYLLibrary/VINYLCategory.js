...
import VINYLCategoryForm from './VINYLCategoryForm';
import { GetCategories, DeleteCategory, UpdateCategoryStatus } from '../../store/lib/api';

const columns = [
    {key: 'num', label: 'No.', width: 70},
    {key: 'name', label: '카테고리명', sort: true, searchable: true},
...
const VINYLCategory = () => {
    ...
    const handleChangeStatus = (camp) => {
        UpdateCategoryStatus(camp.id, {status: camp.status === 'ACTIVE' ? 'DISABLE' : 'ACTIVE'})
        .then(res => {
            getData(tableState);
        }).catch(err => {
            console.log(err);
    ...
    return (
        <div>
            ...
            <h4 className="mb-5">VINYL 장르 카테고리</h4>
