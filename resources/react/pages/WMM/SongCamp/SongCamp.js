...
import SongCampEdit from './SongCampEdit';
import SongCampSubmit from './SongCampSubmit';
import SongCampView from './SongCampView';
...
const SongCamp = () => {
    ...
    return (
        <div id="page-songcamp" className="pt-5 mb-50p">
            {submitting && <CircleFullSpinner size="full"/>}
            <div className="section-container">
                <div className="songcamp-container">
                    <h2 className="text-center font-weight-bold text-ttnorm-bd mb-4">SONGCAMP</h2>
                    ...
                    {campaigns && !!campaigns.length &&
                        <>
                            ...
                            {step === 1 && <SongCampEdit campaign={campaigns[tab]} onSubmit={handleRequest}/>}
                            {step === 2 && <SongCampSubmit tab={tab} userInfo={userState} campaign={campaigns[tab]} onSubmit={handleRequest}/>}
                            {step === 3 && <SongCampView tab={tab} userInfo={userState} campaign={campaigns[tab]} onSubmit={handleRequest}/>}
                        </>
                    }
                </div>
            </div>
